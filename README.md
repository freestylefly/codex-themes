...
当前安装包尚未完成 Apple 签名与公证。浏览器下载 DMG 时，macOS 会给应用添加 `com.apple.quarantine` 隔离属性；Gatekeeper 无法验证开发者身份时，有时会显示“应用已损坏，无法打开”。这通常是隔离策略造成的拦截，并不代表安装包真的损坏。

请确认安装包来自本项目官网或 [GitHub Latest Release](https://github.com/freestylefly/codex-themes/releases/latest)，然后：

1. 打开最新版 DMG，把 **Codex Themes.app** 拖入“应用程序”，选择覆盖旧版本。
2. 打开“终端”，执行：

```bash
xattr -dr com.apple.quarantine "/Applications/Codex Themes.app"
```

3. 重新打开 `/Applications/Codex Themes.app` 即可。

这条命令只移除 Codex Themes 的下载隔离属性，不会全局关闭 macOS Gatekeeper。请勿对来源不明的应用执行。

### 新版 ChatGPT 桌面端模式提示

新版桌面应用同时包含 **ChatGPT、Work 和 Codex**。Codex Themes 只会装饰 Codex，不会修改 ChatGPT 或 Work 对话。

- 点击“应用主题”后，如果桌面应用停留在 ChatGPT / Work，Codex Themes 会自动切换到 Codex，再完成主题注入。
- 自动切换会通过官方 `codex://threads/new` 深链打开一个新的本地 Codex 对话；原来的 ChatGPT / Work 对话不会丢失。
- 之后主动切回 ChatGPT / Work 时，主题会自动暂停；再次进入 Codex 后会自动恢复。
- 如果自动切换超时，请点击桌面应用左上角的“ChatGPT”模式菜单，手动选择“Codex”，然后重新应用主题。

判断方法很简单：顶部出现 `Chat / Work` 切换时处于 ChatGPT 模式；左上角显示 `Codex` 时才是主题支持的界面。模式区别可参考 [OpenAI 官方说明](https://help.openai.com/en/articles/20001275-chatgpt-work-and-codex)。

### 主题图片与原生界面

- `hero` / `wallpaper` 必须是纯背景画面，不能把侧栏、顶部栏、卡片、对话气泡或输入框画进图片；这些交互区域始终由真实 Codex 界面提供。
- `preview.png` 可以是带完整界面的展示截图，但它只用于画廊预览，不能作为可应用主题的主图。
- 在另一台电脑使用已购主题时，客户端会先自动下载完整主题包，再应用真实主图；展示占位图不会被注入 Codex。
- 如果升级前曾应用异常主题，请升级到最新版后回到主题画廊重新点击“使用”。新版会重新下载并优先使用正确的已购资源。
...
| 目标 | 需要修改的位置 |
| --- | --- |
| 官网域名、GitHub 仓库、最新版下载入口 | `web/src/config.ts` |
...
主题资源建议明确分工：`hero` / `wallpaper` 只放纯插画、照片或纹理背景，`preview.png` 才负责展示应用后的完整效果。不要让 `hero` 指向带有模拟侧栏、顶部栏、卡片或输入框的截图，否则图片中的假界面会与真实 Codex 界面重叠。AI 生成链路也应保留同样的“纯背景、无 UI、无文字”约束。
