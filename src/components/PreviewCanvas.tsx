 ...
 */
export function PreviewCanvas({ theme, heroUrl, wallpaperUrl, stampUrl, fidelity = "compact" }: PreviewCanvasProps) {
  ...
  const shellArtUrl = theme.layout === "full-canvas"
    ? artUrl
    : theme.wallpaper.enabled
      ? wallpaperUrl ?? undefined
      : undefined;
  // Theme hero heights target the full Codex window (200–360px). The preview
  // is a reduced canvas, so using that value directly pushes the cards and
  // composer below its clipped edge. Keep the same proportion at preview size.
  const previewHeroHeight = fidelity === "app"
    ? theme.hero.height
    : Math.round(theme.hero.height * 0.55);
  ...
  return (
    <div className={`preview-frame preview-frame--${fidelity}`}>
      ...
      >
        {shellArtUrl && (
          <div className="mock-bg" style={{ backgroundImage: `url("${shellArtUrl}")` }} />
        )}
        <div
          className="mock-tint"
          style={{
        ...
        <div className="mock-inner">
          <CodexPreviewSidebar colors={c} page="home" />
          <section className="mock-app-surface">
            <CodexPreviewHeader colors={c} page="home" />
