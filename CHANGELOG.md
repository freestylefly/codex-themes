...
本次版本为 macOS 安装包加入全新的品牌化安装界面，提升首次安装引导的清晰度与一致性。

### 优化

- macOS DMG 使用全新的深夜蓝与暖金视觉，延续 Codex Themes 官网和客户端品牌风格。
- 重新设计应用到 Applications 的拖拽引导，安装路径更直观。
- 调整安装窗口尺寸、图标位置与文字对比度，提升不同 Mac 屏幕上的可读性。
