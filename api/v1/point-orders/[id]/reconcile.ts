...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { data: order, error } = await supabase
    ...
    .eq("user_id", user.id)
    .single();
  if (error || !order) return res.status(404).json({ error: "Point order not found" });
  ...
  if ((tradeStatus === "TRADE_SUCCESS" || tradeStatus === "TRADE_FINISHED") && tradeNo) {
    ...
    const updated = {
      ...
      point_packs: { name: (order.point_packs as unknown as { name: string } | null)?.name },
    };
    return res.status(200).json(mapPointOrder(updated));
  }
